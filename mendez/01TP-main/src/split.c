...
char **split(const char *string, char separador) {
  ...
  // armamos el char**(o el vector de char*)
  int cantidad_de_elementos_vec =
      contador_de_separadores(string, separador) + 1;
  char **vec_de_punteros = (char **)malloc(
      (size_t)(cantidad_de_elementos_vec + 1) *
      sizeof(char *)); /// hago un reecasteo del tipo de dato porque sino me
                       /// tira error el parametro -werror(que toma los warnings
                       /// como errores)
  ...
  for (i = 0; string[i] != '\0' && puedo_continuar != 1; i++) {
    if (string[i] == separador) {
      int longitud_del_char_ptr = i - posicion_inicial_de_palabra;
      vec_de_punteros[indice_del_vector] = (char *)malloc(
          (size_t)(longitud_del_char_ptr + 1) *
          sizeof(char)); // hago un reecasteo del tipo de dato porque sino me
                         // tira error el parametro -werror(que toma los
      ...
      if (vec_de_punteros[indice_del_vector] == NULL) {
        ...
        strncpy(vec_de_punteros[indice_del_vector],
                ...
                (size_t)longitud_del_char_ptr); // hago un reecasteo del tipo de
                                             // dato porque sino me tira error
                                             // el parametro -werror(que toma
                                             // los warnings como errores)
        vec_de_punteros[indice_del_vector][longitud_del_char_ptr] = '\0';
  ...
  if (!puedo_continuar) {
    size_t longitud_restante =
        strlen(string) -
        (size_t)posicion_inicial_de_palabra; // hago un reecasteo del tipo de
                                             // dato porque sino me tira error
                                             // el parametro -werror(que toma
                                             // los warnings como errores)
