			     ...
			     const struct pokemon_info *pokemon)
{
	if (!jugador || !pokemon)
		return false;
	if (otro_jugador && otro_jugador->pokemon == pokemon)
		return false;
				...
				unsigned posicion)
{
	...
	if (jugador->tamanio_pista >= jugador->capacidad_pista) {
		jugador->capacidad_pista = jugador->capacidad_pista == 0 ?
						   1 :
						   jugador->capacidad_pista * 2;
		enum TP_OBSTACULO *nueva_pista = realloc(
			jugador->pista,
			jugador->capacidad_pista * sizeof(enum TP_OBSTACULO));
		if (!nueva_pista)
			return 0;
...
{
	if (!jugador || posicion >= jugador->tamanio_pista)
		return 0;
...
{
	if (!jugador || !jugador->pokemon || jugador->tamanio_pista == 0)
		return NULL;
