...
void crear_pista_rival(TP *tp, Dificultad dif)
{
	...
	if (!nombre_pokemon_rival ||
	    !tp_seleccionar_pokemon(tp, JUGADOR_2, nombre_pokemon_rival)) {
...
char *seleccionar_nombre_azar(TP *tp)
{
	...
	size_t longitud = fin_nombre ? (size_t)(fin_nombre - nombre_actual) :
				       strlen(nombre_actual);
...
bool pokemon_existe(TP *tp, const char *nombre)
{
	...
	for (int i = 0; nombres_lower[i]; i++) {
		nombres_lower[i] =
			(char)tolower((unsigned char)nombres_lower[i]);
	}
	...
	while (!juego_terminado) {
		...
		case 4:
			if (nivel_dificultad == 0 ||
				...
				Dificultad dif =
					dificultades[nivel_dificultad - 1];
		...
		case 5:
			if (nivel_dificultad == 0 ||
				...
				ejecutar_carrera(
					tp, dificultades[nivel_dificultad - 1]);
			}
			break;
		case 6:
