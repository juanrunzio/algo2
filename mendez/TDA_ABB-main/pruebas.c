...
void no_puedo_crear_arbol_con_comparador_null()
{
	pa2m_afirmar(abb_crear(NULL) == NULL,
		     "No puedo crear arbol con comparador NULL");
}
...
void arbol_creado_estructuras_inicializadas()
{
	...
	pa2m_afirmar(nuevo_arbol != NULL,
		     "Se pudo reservar memoria para el nuevo arbol");
	pa2m_afirmar(abb_tamanio(nuevo_arbol) == 0,
		     "La cantidad de elementos es cero al crear el arbol");
...
{
	int numero = 10;
	pa2m_afirmar(abb_insertar(NULL, &numero) == NULL,
		     "No puedo insertar elemento en arbol NULL");
}
...
void puedo_insertar_elemento_en_arbol()
{
	...
	pa2m_afirmar(nuevo_arbol->tamanio == 1,
		     "Se pudo insertar elemento en arbol vacio");
	nuevo_arbol = abb_insertar(nuevo_arbol, &numero_20);
	pa2m_afirmar(nuevo_arbol->tamanio == 2,
		     "Se pudo insertar elemento en arbol no vacio");
	nuevo_arbol = abb_insertar(nuevo_arbol, &numero_5);
	pa2m_afirmar(nuevo_arbol->tamanio == 3,
		     "Se pudo insertar elemento en arbol no vacio");
	nuevo_arbol = abb_insertar(nuevo_arbol, &numero_30);
	pa2m_afirmar(nuevo_arbol->tamanio == 4,
		     "Se pudo insertar elemento en arbol no vacio");
	nuevo_arbol = abb_insertar(nuevo_arbol, &numero_1);
	pa2m_afirmar(abb_tamanio(nuevo_arbol) == 5,
		     "Se pudo insertar elemento en arbol no vacio");
	abb_destruir(nuevo_arbol);
}
...
void no_puedo_eliminar_elemento_que_no_existe()
{
	...
	abb_t *nuevo_arbol = abb_crear(comparador);
	nuevo_arbol = abb_insertar(nuevo_arbol, &numero_10);
	pa2m_afirmar(abb_quitar(nuevo_arbol, &numero_20) == NULL,
		     "No puedo eliminar elemento que no existe en el arbol");
	abb_destruir(nuevo_arbol);
}
...
void puedo_eliminar_elemento_del_arbol()
{
	...
	nuevo_arbol = abb_insertar(nuevo_arbol, &numero_20);
	nuevo_arbol = abb_insertar(nuevo_arbol, &numero_30);
	pa2m_afirmar(abb_quitar(nuevo_arbol, &numero_30) == &numero_30,
		     "Quita un primer elemento y lo devuelve");
	pa2m_afirmar(abb_quitar(nuevo_arbol, &numero_20) == &numero_20,
		     "Quita un segundo elemento y lo devuelve");
	pa2m_afirmar(abb_quitar(nuevo_arbol, &numero_10) == &numero_10,
		     "Quita un tercero elemento y lo devuelve");
	pa2m_afirmar(abb_tamanio(nuevo_arbol) == 0,
		     "Tamanio del arbol disminuye en uno");
	abb_destruir(nuevo_arbol);
}
...
void no_puedo_buscar_elemento_null()
{
	...
	nuevo_arbol = abb_insertar(nuevo_arbol, &numero_10);
	nuevo_arbol = abb_insertar(nuevo_arbol, &numero_20);
	pa2m_afirmar(abb_buscar(nuevo_arbol, NULL) == NULL,
		     "No puedo buscar elemento NULL");
	abb_destruir(nuevo_arbol);
}
...
void no_puedo_iterar_funcion_null()
{
	...
	pa2m_afirmar(abb_con_cada_elemento(nuevo_arbol, INORDEN, NULL, &null) ==
			     0,
		     "No puedo iterar arbol con funcion NULL");
	abb_destruir(nuevo_arbol);
}
...
int main()
{
	...
	pa2m_nuevo_grupo(
		"Pruebas de guardar elementos en array con recorrido inorden");
	guardo_elementos_en_array_in_orden();

	pa2m_nuevo_grupo(
		"Pruebas de guardar elementos en array con recorrido preorden");
	guardo_elementos_en_array_pre_orden();

	pa2m_nuevo_grupo(
		"Pruebas de guardar elementos en array con recorrido postorden");
