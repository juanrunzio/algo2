...
// Me equivoque y hice una implementacion de un hash abierto en vez de uno
// cerrado por lo tanto tuve que rehacer toda la implementacion. pero lo dejo
// por las dudas para tenerlo :(
...
struct nodo *insertar_nodo(struct nodo *nodo, const char *clave, void *elemento,
  ...
  nodo->siguiente =
      insertar_nodo(nodo->siguiente, clave, elemento, anterior, tope);
  return nodo;
}
...
hash_t *rehash(hash_t *hash) {
  ...
  for (int i = 0; i < hash->capacidad; i++) {
    ...
    while (nodo_actual) {
      size_t posicion = funcion_de_hash(nodo_actual->clave, nueva_capacidad);
      struct nodo *aux = nodo_actual;
      nuevo_array[posicion] =
          insertar_nodo(nuevo_array[posicion], nodo_actual->clave,
                        nodo_actual->elemento, NULL, &tope_aux);
...
hash_t *hash_insertar(hash_t *hash, const char *clave, void *elemento,
  ...
  if (porcentaje >= FACTOR_CARGA_MAXIMA * 100) {
    hash_t *nuevo_hash = rehash(hash);
    if (!nuevo_hash)
      return NULL;
    hash = nuevo_hash;
  }
  size_t posicion = funcion_de_hash((char *)clave, hash->capacidad);
  hash->array[posicion] = insertar_nodo(hash->array[posicion], clave, elemento,
                                        anterior, &(hash->tope));
  return hash;
}

struct nodo *eliminar_nodo(struct nodo *nodo, const char *clave,
                           void **elemento_encontrado, bool *eliminado) {
  if (!nodo)
    return NULL;
  ...
  nodo->siguiente =
      eliminar_nodo(nodo->siguiente, clave, elemento_encontrado, eliminado);
  return nodo;
}
...
void *hash_quitar(hash_t *hash, const char *clave) {
  ...
  hash->array[posicion] =
      eliminar_nodo(hash->array[posicion], clave, &elemento, &eliminado);
  if (eliminado)
    hash->tope--;
  return elemento;
}

void buscar_nodo(struct nodo *nodo, const char *clave,
                 void **elemento_encontrado, bool *encontrado) {
  if (!nodo)
    return;
  if (strcmp(nodo->clave, clave) == 0) {
    *elemento_encontrado = nodo->elemento;
