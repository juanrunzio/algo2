...
lista_t *lista_insertar_nodo_posicion(lista_t *lista, nodo_t *nuevo_nodo,
				      size_t posicion)
{
	if (posicion == 0) {
...
void *lista_quitar_de_posicion(lista_t *lista, size_t posicion)
{
	...
	if ((posicion >= lista->longitud) || (lista->longitud == 1))
...
{
	if (!lista || (posicion >= lista->longitud) || lista_vacia(lista))
		return NULL;
	nodo_t *nodo_actual = lista_buscar_nodo(lista, posicion);
	return nodo_actual->elemento;
	// esto es lo que hice antes, pero falla
...
bool lista_vacia(lista_t *lista)
{
	if (!lista)
		return true;
	if ((lista->longitud == 0) && lista->nodo_fin == NULL &&
	    lista->nodo_inicio == NULL)
		return true;
	return false;
}
